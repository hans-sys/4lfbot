...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Hans\n' 
            + 'ORG: Pengembang XBot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=6282285537176:+62 822-8553-7176\n' 
            + 'END:VCARD' 
blocked = []   
prefix = '#'
limitawal = 30
...
/******** OWNER NUMBER**********/
const ownerNumber = ["6282285537176@s.whatsapp.net","6282285537176@s.whatsapp.net"] 
/************************************/
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hallo @${num.split('@')[0]}\Selamat datang di group *${mdata.subject}*  Yang Betah Disini Ya Bang`
				...
				teks = `Selamat Tinggal Sobat🥳 @${num.split('@')[0]} Btw Jan Lupa Follow ig @hansreal.ct`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
           ...
           return false
            ...
            if (bad.includes(messagesC)) {
                if (!isGroupAdmins) {
                    try { 
                        ...
                        } catch { client.sendMessage(from, `Untung Saya Bukan Admin,Kalo Tidak Anda Sudah Saya Kick!`, text , {quoted : mek}) }
                } else {
			...
			switch(command) { 
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					setTimeout( () => {
					client.sendMessage(from, ngebuff, image, { caption: '_Tebak bro!!! gak bisa jawab donasi ya:v_', quoted: Hii }) // ur cods
					}, 0) // 1000 = 1s,
				...
				case 'pornhub':
					...
					var porn = gh.split("&")[0];
					var hub = gh.split("&")[1];
					if (args.length < 1) return reply(`「❗」Contoh : ${prefix}Ini Telah Dihapus oleh Hans`)
					reply(ind.wait())
				...
				case 'nulis':
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					if (args.length < 1) return reply(`Teksnya mana bang jago? Contoh : ${prefix}nulis hans pro sangat`)
					nul = body.slice(7)
            ...
            case 'bitly':
               ...
               case 'help': 
                ...
                if (!nomerr) return reply(`maaf format salah\nmasukan parameter yang benar\ncontoh : ${prefix}giftlimit @6282285537176 20`)
